...
public:
  ...
  [[nodiscard]] constexpr Node_Sptr Ptr() const & {
    return std::make_shared<Node>(*this);
  }
  [[nodiscard]] constexpr Node_Wptr getThis() {
  ...
  constexpr double ucb1Score(const Player &player,
...
private:
  ...
  std::map<Player, double> wins{{Player{Player::Mark::X}, 0.},
                                {Player{Player::Mark::O}, 0.}};
};
} // namespace MCTS
#endif // !MCTS_HPP
