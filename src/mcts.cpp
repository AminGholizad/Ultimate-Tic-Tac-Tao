...
#include <iostream>
namespace MCTS {
template <class State> typename Node<State>::Node_Sptr Node<State>::getChild() const & {
    return (std::all_of(children.begin(), children.end(),
                        [](auto child) { return child->visits > 0; }))
               ? ucb1()
               : getRandomChild();
}
template <class State> typename Node<State>::Node_Sptr Node<State>::ucb1() const & {
    const auto lnTotal = log(visits);
    const auto player = state.getPlayer();
    return *std::max_element(
        children.begin(), children.end(), [player, lnTotal](auto node_a, auto node_b) {
            return node_a->ucb1Score(player, lnTotal) < node_b->ucb1Score(player, lnTotal);
        });
}
template <class State> typename Node<State>::Node_Sptr Node<State>::bestChild() const & {
    const auto player = state.getPlayer();
    return *std::max_element(children.begin(), children.end(), [player](auto node_a, auto node_b) {
...
typename Node<State>::Node_Sptr Node<State>::choose_move(const Timer::milliseconds_t &duration) {
