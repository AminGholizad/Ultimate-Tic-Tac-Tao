...
void mctsVSmcts(Timer::milliseconds_t time1, Timer::milliseconds_t time2) {
...
void userVSmcts(Timer::milliseconds_t time) {
    auto tmp = MCTS::Node<UTTT::State>().Ptr();
    tmp->debugBoard();
...
void userVSngm(Timer::milliseconds_t time) {
...
void ngmVSngm(Timer::milliseconds_t time1, Timer::milliseconds_t time2) {
    auto tmp = NEGAMAX::Negamax<UTTT::State>();
    tmp.debugBoard();
    auto time = time1;
    while (!tmp.isOver()) {
...
void mctsVSngm(Timer::milliseconds_t time1, Timer::milliseconds_t time2) {
